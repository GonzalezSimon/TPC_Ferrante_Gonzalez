...
namespace Negocio
{
    public class NegocioServContratado
    {
        public List<ServicioContratado> listar(string consulta = "")
        {
            ...
            try
            {
                datos.setearConsulta("select s.IDServicio, s.IDTipoServicio, ts.TipoServicio, ts.Estado, s.Precio, s.Descripcion, s.Slots, s.EstadoServicio, u.IDUsuario, U.Usuario, u.UPassword, u.Apellido, u.Nombre, u.Telefono, u.Mail, l.IDLocalidad, l.Nombre, l.Estado, p.IDPais, p.Nombre, p.Estado, tu.IDTipoUsuario, tu.TipoUsuario, tu.Estado, u.URLUsuario, u.FechaCreacion, u.EstadoUsuario, sc.FechaInicio, sc.Fechafin, sc.GrupoSoporte, sc.EstadoSXC from ServicioContratado as sc inner join Servicios as s on s.IDServicio = sc.IDServicio inner join TipoServicio as ts on ts.IDTipoServicio = s.IDTipoServicio inner join Usuarios as U on u.IDUsuario = sc.IDUsuario inner join TipoUsuario as tu on tu.IDTipoUsuario = u.IDTipoUsuario inner join localidades as L on L.IDlocalidad = u.IDLocalidad inner join paises as p on p.IDPais = L.IDPais" + consulta);
